 ...
 * Env: SPREADSHEET_ID, GOOGLE_SHEETS_API_KEY (read), GOOGLE_SERVICE_ACCOUNT_KEY (write, JSON string).
 */
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (!sheets) return res.status(503).json({ error: "RSVP write not configured (missing GOOGLE_SERVICE_ACCOUNT_KEY)" });
    const { eventId, personId, fullName, status } = req.body || {};
    if (!eventId || !personId) return res.status(400).json({ error: "eventId and personId required" });
    const now = new Date().toISOString();
    const row = [eventId, personId, fullName || "", status || "going", now, now];
    try {
      ...
      return res.status(201).json({ eventId, personId, fullName: fullName || "", status: status || "going", createdAt: now, updatedAt: now });
    } catch (e) {
